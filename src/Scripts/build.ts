...
export async function build(push: boolean = false): Promise<DeploymentEndpoint[]> {
    ...
    async function buildProject(project: TWProjectWithDependencies, outPath: string): Promise<void> {
        ...
        if (project.kind == TWProjectKind.TypeScript) {
            ...
            // end in d.ts
            const total = program.getProgram().getRootFileNames().filter(f => !f.endsWith('.d.ts')).length;
            let current = 0;
            ...
            const emitResult = program.emit(undefined, (fileName: string, text: string, ...others) => {
                if (fileName.includes('tsbuildinfo')) {
                    FS.writeFile(fileName, text, () => {});
                }
            }, undefined, undefined, {
                before: [
                    TWThingTransformerFactory(program.getProgram(), project.path, false, false, twConfig)
                ],
                after: [
                    TWThingTransformerFactory(program.getProgram(), project.path, true, false, twConfig)
                ]
            });
